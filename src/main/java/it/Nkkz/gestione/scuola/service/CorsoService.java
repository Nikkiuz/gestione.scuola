...
public class CorsoService {
	...
	public List<CorsoResponseDTO> getCorsiByGiornoEOrario(Giorno giorno, String orario) {
	...
	public CorsoResponseDTO creaCorso(CorsoRequestDTO request) {
		Aula aula = aulaRepository.findById(request.getAulaId())
			.orElseThrow(() -> new EntityNotFoundException("Aula non trovata con ID: " + request.getAulaId()));

		Insegnante insegnante = insegnanteRepository.findById(request.getInsegnanteId())
			.orElseThrow(() -> new EntityNotFoundException("Insegnante non trovato con ID: " + request.getInsegnanteId()));
		...
		BeanUtils.copyProperties(request, corso, "giorno", "secondoGiorno", "aulaId", "insegnanteId", "studentiIds");
	...
	public CorsoResponseDTO modificaCorso(Long id, CorsoRequestDTO request) {
		...
		// ✅ Query aggiornata con enum Giorno
		List<Corso> corsiEsistenti = corsoRepository.findByAulaIdAndGiornoAndOrarioAndAttivoTrue(
			aula.getId(), request.getGiorno(), request.getOrario());
		...
		BeanUtils.copyProperties(request, corso, "id", "giorno", "secondoGiorno", "aulaId", "insegnanteId", "studentiIds");

		// ✅ Assegna correttamente i campi enum e relazionali
		corso.setGiorno(request.getGiorno());
		corso.setSecondoGiorno(request.getSecondoGiorno());
	...
	public void generaCorsiAutomaticamente() {
		...
		while (!studentiDisponibili.isEmpty()) {
			...
			Optional<Giorno> giornoCompatibile = studente.getGiorniPreferiti().stream().findFirst();
			...
			while (index < listaGruppo.size()) {
				...
				if ("2 volte a settimana".equalsIgnoreCase(studente.getTipoCorsoGruppo())) {
					Set<Giorno> giorniComuni = new HashSet<>(sottoGruppo.get(0).getGiorniPreferiti());
					for (Studente s : sottoGruppo) {
						giorniComuni.retainAll(s.getGiorniPreferiti());
					}


					Set<String> orariComuni = new HashSet<>(sottoGruppo.get(0).getFasceOrariePreferite());
					for (Studente s : sottoGruppo) {
						orariComuni.retainAll(s.getFasceOrariePreferite());
					}
					...
					Giorno secondoGiorno = giorniComuni.iterator().next();
